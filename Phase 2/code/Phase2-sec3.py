...
def add_ovs_bridge_form(screen):
    """
    Ask user for a bridge name, create it.
    """
...
def add_port_to_bridge_form(screen):
    """
    Ask user for bridge and port, then add.
    """
    br = input_box(screen, "Enter OVS bridge name:")
    if br is None:
        return
    prt = input_box(screen, "Enter port (interface) name to add:")
    if prt is None:
        return
    try:
        add_port_to_bridge(br, prt)
        message_box(screen, f"Added port '{prt}' to bridge '{br}'.")
    except Exception as e:
        phase3_logger.error(f"Error adding port {prt} to bridge {br}: {e}")
...
def set_port_trunk_form(screen):
    """
    Ask user for port name and VLAN list for trunk mode.
    """
    prt = input_box(screen, "Enter port name to set as trunk:")
    if prt is None:
        return
    vlans = input_box(screen, "Enter comma-separated VLAN IDs (e.g. 10,20,30 or 1-4094):")
    if vlans is None:
        return
    try:
...
def set_port_access_form(screen):
    """
    Ask user for port name and single VLAN ID for access mode.
    """
...
def main(screen):
    """
    Entry point: Check for root, then go to main_menu.
    """
    if os.geteuid() != 0:
        screen.clear()
        screen.border(0)
